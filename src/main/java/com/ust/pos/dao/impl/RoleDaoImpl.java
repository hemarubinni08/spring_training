...
public class RoleDaoImpl implements RoleDao {
    ...
    public List<Role> findAllRolesJdbc() {
        ...
        List<Role> roles = jdbcTemplate.query(sqlQuery, new BeanPropertyRowMapper(Role.class));
        return roles;
    }
    ...
    @Override
    public boolean updateRoleJdbc(Role role) {
        String sqlQuery = "UPDATE ROLE SET NAME = ? WHERE ID = ?";
        int count = jdbcTemplate.update(sqlQuery, role.getName(), role.getId());
        if (count > 0) return true;
        else return false;
    }
